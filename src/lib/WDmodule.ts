...
export const wdModule = (baseUrl: string) => {
   ...
   * @returns 页面标签列表
   */
  const getTags = async (page: string): Promise<string[]> => {
    const pageTag: AjaxResponse = await post.ajaxPost(
      { pageId: await post.getPageId(page) },
      "pagetags/PageTagsModule",
    );
    const dom: JSDOM = new JSDOM(pageTag.body);
    const tagValue: string = dom.window.document.querySelector("input#page-tags-input")?.getAttribute("value") || "";
    return tagValue.split(" ").filter((tag: string) => tag.length > 0);
  };
