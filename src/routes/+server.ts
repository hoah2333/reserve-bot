...
async function mainFunc(): Promise<void> {
    ...
    async function insertDB(insertData: PageType): Promise<void> {
        ...
        if (dataInDB == null) {
            ...
            for (const key in insertData) {
                if (
                    insertData[key as keyof PageType].toLocaleString() !==
                    dataInDB[key as keyof PageType].toLocaleString()
                ) {
     ...
     * 处理所捕获的翻译预定页面。包括：
     * 1. 检查页面是否已翻译或无原文
     * 2. 当页面建立超过 30 日时自动将页面分类改为 outdated:
    ...
    async function reserveHandler(reserve: PageType[], isOutdate = false): Promise<void> {
        reserve.forEach(async (page: PageType): Promise<void> => {
            ...
            let pageUnixName: string = page.reservePage.replace(/^(reserve:)|(outdated:)/, "");
            ...
            if (!isOutdate && timeAgo > 1000 * 60 * 60 * 24 * 30) {
                console.log(
                    `${new Date().toLocaleString()} - ${page.reservePage} 页面已过期，正在重命名`
                );

                if (
                    (await techSite.searchPage(techSiteId, `outdate:${pageUnixName}`)).find(
                        (result: Record<string, string>) =>
                            result.unix_name === `outdate:${pageUnixName}`
                    )
                ) {
                    console.log(
                        `${new Date().toLocaleString()} - ${page.reservePage} 检测到重复过期预定，正在删除较旧过期预定`
                    );
